...
class Level5VM @Inject constructor(
    ...
    fun getData() {
        ...
        levelDBRepository.getData("Books").addOnSuccessListener {
            for (document in it) {
                questions.add(
                    Quiz(
                        document.data.getValue("book").toString(),
    ...
    fun checkAnswersNum(name: String, answer: String) {
        dataBase.data("Books").whereEqualTo("book", name).get()
            .addOnSuccessListener {
                if (it != null) {
                    for (documents in it) {
                        if (documents.getString("year").toString().equals(answer)) {
