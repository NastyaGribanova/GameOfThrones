...
class Level8VM @Inject constructor(
    ...
    fun getData() {
        ...
        levelDBRepository.getData("Killers").addOnSuccessListener {
            for (document in it) {
                questions.add(
                    Quiz(
                        document.data.getValue("victim").toString(),
                        document.data.get("killer").toString()
                    )
                )
            }
    ...
    fun checkAnswersNum(name: String, answer: String) {
        dataBase.data("Killers").whereEqualTo("victim", name).get()
            .addOnSuccessListener {
                if (it != null) {
                    for (documents in it) {
                        if (documents.getString("killer").toString().equals(answer)) {
