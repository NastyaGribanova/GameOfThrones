...
class Level9VM @Inject constructor(
    ...
    fun getData() {
        ...
        levelDBRepository.getData("Swords").addOnSuccessListener {
            for (document in it) {
                questions.add(
                    Quiz(
                        document.data.getValue("sword").toString(),
                        document.data.get("person").toString()
                    )
                )
            }
    ...
    fun checkAnswersNum(name: String, answer: String) {
        dataBase.data("Swords").whereEqualTo("sword", name).get()
            .addOnSuccessListener {
                if (it != null) {
                    for (documents in it) {
                        if (documents.getString("person").toString().equals(answer)) {
