...
class Level6VM @Inject constructor(
    ...
    fun getData() {
        ...
        levelDBRepository.getData("DeadOrAlive").addOnSuccessListener {
            for (document in it) {
                questions.add(
                    Quiz(
    ...
    fun checkAnswersNum(name: String, answer: String) {
        dataBase.data("DeadOrAlive").whereEqualTo("name", name).get()
            .addOnSuccessListener {
                if (it != null) {
                    for (documents in it) {
                        if (documents.getString("status").toString().equals(answer)) {
