...
class Level4VM @Inject constructor(
    ...
    fun getData() {
        ...
        levelDBRepository.getData("Actors").addOnSuccessListener {
            for (document in it) {
                questions.add(
                    Quiz(
                        document.data.getValue("character").toString(),
                        document.data.get("actor").toString()
                    )
                )
            }
    ...
    fun checkAnswersNum(name: String, answer: String) {
        dataBase.data("Actors").whereEqualTo("character", name).get()
            .addOnSuccessListener {
                if (it != null) {
                    for (documents in it) {
                        if (documents.getString("actor").toString().equals(answer)) {
