...
class Level10VM @Inject constructor(
    ...
    fun getData() {
        ...
        levelDBRepository.getData("TheCities").addOnSuccessListener {
            for (document in it) {
                questions.add(
                    Quiz(
                        document.data.getValue("city").toString(),
                        document.data.get("region").toString()
                    )
                )
            }
    ...
    fun checkAnswersNum(name: String, answer: String) {
        dataBase.data("TheCities").whereEqualTo("city", name).get()
            .addOnSuccessListener {
                if (it != null) {
                    for (documents in it) {
                        if (documents.getString("region").toString().equals(answer)) {
